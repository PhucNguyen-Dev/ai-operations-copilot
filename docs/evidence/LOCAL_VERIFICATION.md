...
## 2026-09-19 addendum — approval inbox (same day, second pass)

A follow-up change rebuilt the human side of the approval protocol: a **pending-approvals inbox** at `/agent/approvals` (Operations/Admin nav entry "Agent Approvals"), with args snapshots, an optional decision note, self-decision and conflict guards mirrored from the endpoint, and an Ask X link from awaiting-approval cards. The decision API is unchanged and remains the sole enforcement point. Verified on the resulting tree: lint and typecheck clean, **280/280 unit tests across 24 files** (seven new client-helper tests for the decision mapping), n8n validation 4 workflows / 3 warnings, and a passing production build whose route manifest includes `/agent/approvals`. Browser acceptance of the inbox remains part of the live matrix (requires configured services), as below.
