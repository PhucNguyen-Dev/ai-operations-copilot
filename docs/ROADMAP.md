...
| F — Real external lead trigger | Webhook source with signature validation into the governed pipeline (reuses `webhook-signing.ts`) | ✅ Done (source-agnostic signed webhook `POST /api/webhooks/lead`; migration 013 idempotency; live verification pending — runbook below) |
