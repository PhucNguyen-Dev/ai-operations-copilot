...
| C — Agent behavior evaluation | Scripted-fake-model unit evals in CI + ~10 real-Gemini scenarios on a **schedule** (not per-PR — cost/flakiness), machine-readable results | ✅ Done (9/9 scenarios pass; **nightly in CI** via `.github/workflows/agent-evals.yml` — first scheduled run green with results artifact, 2026-09-13) |
